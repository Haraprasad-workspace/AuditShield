    ...
    for (const res of auditResults) {
      if (res.risk_level === 'critical') {
        await createLogEntry({
          user_id: userId,
          source: 'google_drive',
          message: `Public Exposure: ${res.name}`,
          risk: 'critical',
          reason: `Asset "${res.name}" is shared globally.`,
          suggestion: "Revoke public access immediately.",
          file_path: res.metadata.webViewLink,
          status: 'open'
        });
      }
    }
...
// --- NEW Action: Revoke Public Access (Make Private) ---
// --- Action: Revoke Public Access (Make Private) ---
export const revokeFileAccess = async (req, res) => {
  ...
  try {
    ...
    // 1. Fetch current permissions to find the 'anyone' permission ID
    ...
    if (!publicPermission) {
      return res.status(404).json({ error: "No public 'anyone' permissions found. File is likely already private." });
    }

    // 2. Delete the public permission
    ...
    // Log the detailed error to your server terminal for debugging
    console.error("❌ [REVOKE_ERROR]:", err.response?.data || err.message);
    
    const errorMessage = err.response?.data?.error?.message || err.message;
    res.status(err.response?.status || 500).json({ 
      error: `Failed to revoke access: ${errorMessage}` 
    });
  }
};

// --- Action: Delete File (Move to Trash) ---
export const deleteDriveFile = async (req, res) => {
  ...
  try {
    ...
    // In Drive API v3, 'trashed' is a property of the file updated via requestBody
...
export const handleGoogleCallback = async (req, res) => {
  ...
  const oauth2Client = new google.auth.OAuth2(
    process.env.GOOGLE_CLIENT_ID, process.env.GOOGLE_CLIENT_SECRET, "http://localhost:5173/drive-callback"
  );
  try {
